...
namespace InnosixWebApp.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            if (userId != null && await UserManager.IsEmailConfirmedAsync(userId))
            {
                // This doesn't count login failures towards account lockout
                // To enable password failures to trigger account lockout, change to shouldLockout: true
                result = await SignInManager.PasswordSignInAsync(model.Email, model.Password, model.RememberMe,
                    shouldLockout: false);
            }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            if (result.Succeeded)
            {
                await UserManager.AddToRoleAsync(user.Id, "TC");
                ...
                return RedirectToAction("ConfirmMessage", "Account");
            }
            AddErrors(result);
        ...
        [AllowAnonymous]
        public async Task<ActionResult> ConfirmMessage()
        {
            return View();
        }
        ...
        public ActionResult PassOwnerShip()
        {
            using (var db = new InnosixEntities())
            {
                var userId = User.Identity.GetUserId();
                ...
                var claimUserEmail = db.AspNetUsers.Find(claim.AspNetUserId).Email;

                var passOwnerShipViewModel = new PassOwnerShipViewModel
                {
                ...
                return View(passOwnerShipViewModel);
            }
        }

        public ActionResult PassOwnerShipDecline()
        {
            using (var db = new InnosixEntities())
            {
                var userId = User.Identity.GetUserId();
